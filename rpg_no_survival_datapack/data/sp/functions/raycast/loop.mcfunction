...
tag @s add user
tag @s add dealt
execute unless entity @a[distance=..3,tag=user] run particle crit ~ ~ ~ 0 0 0 0 3 normal @a
execute as @e[type=!#sp:protect,distance=..10] store result score @s dynamic_health run data get entity @s Health 10
execute as @e[type=!#sp:protect,dx=0,limit=1,sort=nearest,nbt=!{Invulnerable:1b},tag=!amirite_crystal] positioned ~-0.9 ~-0.9 ~-0.9 if entity @s[dx=0] run function sp:guns/entity_hit

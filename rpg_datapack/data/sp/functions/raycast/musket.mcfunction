...
execute as @e[type=!#sp:protect,dx=0,limit=1,sort=nearest,scores={hit=0},nbt=!{Invulnerable:1b}] positioned ~-0.9 ~-0.9 ~-0.9 if entity @s[dx=0] run function sp:guns/entity_hit
...
kill @e[type=area_effect_cloud,tag=bounce,scores={raycast=..0}]
kill @e[type=area_effect_cloud,tag=bounce,scores={bounce=..0}]